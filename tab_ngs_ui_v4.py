...
def render_ngs():
    ...
    if result:
        ...
        with st.container(border=True):
            ...
            display_df["fragment_name"] = display_df["fragment_index"].map(lambda x: f"Fragment {int(x)}")
            ...
            fragment_labels = {r.fragment_name: f"Fragment {int(r.fragment_index)}" for r in result.fragments}
            selected_name = st.selectbox(
                "Choose a fragment",
                options=[r.fragment_name for r in result.fragments],
                index=0,
                format_func=lambda x: fragment_labels.get(x, x),
            )
...
─── 3'""",
            ...
            if result.fragments and result.fragments[-1].is_last_fragment and result.fragments[-1].fragment_length_nt != result.fragments[0].fragment_length_nt:
                st.info(
                    f"The last fragment is shorter than the standard fragment length: "
                    f"{result.fragments[-1].fragment_length_nt} nt total "
                    f"(payload: {result.fragments[-1].payload_length_nt} nt)."
                )
