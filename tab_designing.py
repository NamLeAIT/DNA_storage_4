...
def _render_audio_analysis(report: Optional[Dict[str, Any]], job: str):
    ...
    dur_a = meta_a.get('duration_sec')
    dur_b = meta_b.get('duration_sec')
    dur_diff = abs(float(dur_a) - float(dur_b)) if dur_a is not None and dur_b is not None else None
    sr_a, sr_b = meta_a.get('sample_rate'), meta_b.get('sample_rate')
    ch_a, ch_b = meta_a.get('channels'), meta_b.get('channels')
    br_a, br_b = meta_a.get('bit_rate'), meta_b.get('bit_rate')

    # Single practical similarity score based on structural preservation.
    # The user only sees this one score, while the components stay internal.
    similarity = None
    if report:
        weighted = 0.0
        total_w = 0.0

        if dur_a is not None and dur_b is not None and float(dur_a) > 0:
            rel_diff = abs(float(dur_a) - float(dur_b)) / max(float(dur_a), 1e-9)
            duration_score = max(0.0, 1.0 - min(1.0, rel_diff / 0.02))
            weighted += 0.40 * duration_score
            total_w += 0.40

        if sr_a is not None and sr_b is not None and max(float(sr_a), float(sr_b)) > 0:
            sr_rel = abs(float(sr_a) - float(sr_b)) / max(float(sr_a), float(sr_b))
            sr_score = max(0.0, 1.0 - min(1.0, sr_rel / 0.10))
            weighted += 0.25 * sr_score
            total_w += 0.25

        if ch_a is not None and ch_b is not None:
            ch_score = 1.0 if int(ch_a) == int(ch_b) else max(0.0, 1.0 - min(1.0, abs(int(ch_a) - int(ch_b)) / max(int(ch_a), int(ch_b), 1)))
            weighted += 0.15 * ch_score
            total_w += 0.15

        if br_a is not None and br_b is not None and max(float(br_a), float(br_b)) > 0:
            br_rel = abs(float(br_a) - float(br_b)) / max(float(br_a), float(br_b))
            br_score = max(0.0, 1.0 - min(1.0, br_rel / 0.60))
            weighted += 0.20 * br_score
            total_w += 0.20

        similarity = (100.0 * weighted / total_w) if total_w > 0 else None

    with st.container(border=True):
        st.markdown('### Audio Validation')
        top_left, top_right = st.columns([0.42, 0.58], gap='large')
        with top_left:
            st.metric('Audio similarity', f"{similarity:.1f}/100" if similarity is not None else '—')
            st.caption('This single score summarizes how closely the restored audio matches the original in duration, sampling structure, channel layout, and bitrate profile.')
            if similarity is not None:
                if similarity >= 90:
                    st.success('High structural similarity')
                elif similarity >= 70:
                    st.warning('Moderate structural similarity')
                else:
                    st.error('Low structural similarity')
        with top_right:
...
def _render_video_analysis(report: Optional[Dict[str, Any]], job: str):
    ...
    with st.container(border=True):
        st.markdown('### Video Benchmark Summary')
        st.caption('For video, the system prioritizes compression benchmarking and storage statistics. Visual quality evaluation is intentionally skipped to keep processing practical.')

        m1, m2, m3, m4 = st.columns(4)
        with m1:
            st.metric('Original size', _fmt_bytes(os.path.getsize(in_path)) if in_path and os.path.exists(in_path) else '—')
        with m2:
            st.metric('Restored size', _fmt_bytes(os.path.getsize(out_path)) if out_path and os.path.exists(out_path) else '—')
        with m3:
            st.metric('Duration difference', f"{dur_diff:.3f} s" if dur_diff is not None else '—')
        with m4:
            sel = (((report or {}).get('analysis', {}) or {}).get('selected_method') or ((report or {}).get('benchmark', {}) or {}).get('selected_method'))
            st.metric('Selected method', str(sel) if sel else '—')

        c1, c2 = st.columns(2, gap='large')
        with c1:
            st.markdown('**Original video metadata**')
            st.dataframe(pd.DataFrame([{
                'Duration (s)': (round(float(meta_a.get('duration_sec')), 3) if meta_a.get('duration_sec') is not None else None),
                'Resolution': (f"{meta_a.get('width')} × {meta_a.get('height')}" if meta_a.get('width') and meta_a.get('height') else None),
                'FPS': (round(float(meta_a.get('fps')), 3) if meta_a.get('fps') is not None else None),
                'Video codec': meta_a.get('video_codec'),
                'Audio codec': meta_a.get('audio_codec'),
                'Container': meta_a.get('container'),
                'Bit rate': (f"{int(meta_a.get('bit_rate')):,}" if meta_a.get('bit_rate') is not None else None),
            }]), width='stretch', hide_index=True)
            if in_path and os.path.exists(in_path):
                st.video(in_path)

        with c2:
            st.markdown('**Restored video metadata**')
            st.dataframe(pd.DataFrame([{
                'Duration (s)': (round(float(meta_b.get('duration_sec')), 3) if meta_b.get('duration_sec') is not None else None),
                'Resolution': (f"{meta_b.get('width')} × {meta_b.get('height')}" if meta_b.get('width') and meta_b.get('height') else None),
                'FPS': (round(float(meta_b.get('fps')), 3) if meta_b.get('fps') is not None else None),
                'Video codec': meta_b.get('video_codec'),
                'Audio codec': meta_b.get('audio_codec'),
                'Container': meta_b.get('container'),
                'Bit rate': (f"{int(meta_b.get('bit_rate')):,}" if meta_b.get('bit_rate') is not None else None),
            }]), width='stretch', hide_index=True)
            if out_path and os.path.exists(out_path):
                st.video(out_path)
...
def _render_domain_analysis(report: Optional[Dict[str, Any]], job: str):
    ...
    if domain == 'image':
        _render_image_validation(report, job)
    elif domain == 'text':
...
def _render_summary_card(report: Optional[Dict[str, Any]], detected_domain: str = "—"):
    ...
    rt = (((report or {}).get("runtime", {}) or {}).get("total_runtime_sec") if report else None)
    c_rt, c_main = st.columns([0.9, 3.1], gap="small")
    with c_rt:
        st.metric("Runtime", f"{float(rt):.2f} s" if rt is not None else "—")
    with c_main:
...
def render_designing():
        ...
        with sub_comp:
            ...
            with left_col:
                ...
                with st.container(border=True):
                    ...
                    st.caption("Compression mode is fixed to Auto Benchmark. For video, the benchmark compares Keep vs one H.264 candidate, then evaluates only the selected result.")
    ...
    with tab_ana:
        ...
        total_rt = (((report or {}).get("runtime", {}) or {}).get("total_runtime_sec") if report else None)
        ...
        with top5:
            st.metric("Runtime", f"{float(total_rt):.2f} s" if total_rt is not None else "—")
