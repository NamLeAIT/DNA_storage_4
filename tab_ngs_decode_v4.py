...
def render_ngs_decode():
    st.title("🧬 NGS Decode / Recovery")
    st.caption("Use Standard Test to see whether sequencing-affected DNA can be recovered normally. Use Advanced Rescue to try checksum/manifest-guided recovery on noisy DNA.")
    ...
    with left:
        ...
        with st.container(border=True):
            ...
            if source == "From DNA before FASTQ":
                ...
                if prep:
                    st.success(f"Loaded latest NGS library with {len(prep.fragments):,} fragments")
                else:
                    st.warning("No NGS Prep result found in the current session.")
                if fastq_r1_path and os.path.exists(fastq_r1_path):
                    st.info(f"Loaded latest simulated FASTQ: {os.path.basename(fastq_r1_path)}")
                else:
                ...
                if manifest is not None:
                    ...
                    st.success(f"Loaded manifest with {len(prep.fragments):,} fragments")
                    if r1 is not None:
        ...
        with st.container(border=True):
            ...
            if level == "Standard Test":
                run = st.button("Run Standard Test", type="primary", width="stretch", key="run_std_v4")
            else:
                run = st.button("Run Advanced Rescue", type="primary", width="stretch", key="run_adv_v4")
    ...
    if run:
        ...
        if source == "From DNA before FASTQ":
            ...
            if not payload:
                st.error("No DNA payload before FASTQ is available.")
                return

        elif source in ("From latest FASTQ run", "Import FASTQ files"):
            if prep is None or not fastq_r1_path:
                st.error("Manifest/NGS Prep and at least R1 FASTQ are required.")
                return
            if level == "Standard Test":
        ...
        if level == "Advanced Rescue" and precheck.get("ok"):
    ...
    if report:
        with st.container(border=True):
            title = "### 📈 Standard Recovery Test" if (report.get("mode") == "standard_test") else "### 🛟 Advanced Rescue Report"
            st.markdown(title)
            c1, c2, c3, c4 = st.columns(4)
            with c1:
            ...
            if report.get("mode") == "advanced_rescue":
                ...
                with d2:
                    st.metric("Checksum-like Pass", f"{int(report.get('checksum_pass_like_fragments') or 0):,}")
                with d3:
                    st.metric("Uncertain", f"{int(report.get('uncertain_fragments') or 0):,}")
            st.dataframe(pd.DataFrame(report["rows"]), width="stretch", hide_index=True)
    ...
    if precheck:
        with st.container(border=True):
            st.markdown("### 🧪 Decode Readiness")
            ok = bool(precheck.get("ok"))
            if ok:
                st.success("This payload passes the standard decode/zlib precheck.")
            else:
                st.error("This payload does not pass the standard decode/zlib precheck.")
            st.json(precheck, expanded=False)

    if final_stats:
        with st.container(border=True):
            st.markdown("### 📥 Advanced Final Decode")
            d1, d2, d3 = st.columns(3)
            with d1:
