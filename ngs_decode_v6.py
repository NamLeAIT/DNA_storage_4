...
import gzip, json, os, uuid
from collections import Counter
from typing import Any, Dict, List, Optional, Tuple

import dna_codec
from compressors_v2 import restore_rep
from ngs_prep_v5 import (
    FragmentRecord, NGSPrepResult, clean_dna, gc_ratio, max_homopolymer,
    reconstruct_payload_from_fragments, reverse_complement, shannon_entropy,
    crc_nt, xor_nt_sequences, rs_recover_payload_group
)
...
def load_ngs_prep_result_from_json(path: str) -> NGSPrepResult:
    p = load_json(path)
    fr = [fragment_from_dict(x) for x in p.get("fragments", [])]
    return NGSPrepResult(
        input_sequence_length_nt=int(p.get("input_sequence_length_nt") or 0),
...
def dna_stats(seq: str) -> Dict[str, Any]:
    ...
    return {
        "length_nt": len(seq),
        "gc_ratio": gc_ratio(seq) if seq else 0.0,
        "max_homopolymer": max_homopolymer(seq) if seq else 0,
        "shannon_entropy": shannon_entropy(seq) if seq else 0.0,
    }
...
def _build_consensus_fragments(prep: NGSPrepResult, fastq_r1_path: str, fastq_r2_path: Optional[str] = None):
    ...
    def ensure_arr(fidx: int, length: int):
        if fidx not in base_counts:
            base_counts[fidx] = [Counter() for _ in range(length)]
    for header, seq, qual in iter_fastq_records(fastq_r1_path):
        total_reads += 1
        fidx = parse_header_frag_idx(header)
        if fidx is None or fidx not in frag_map:
            continue
        full = frag_map[fidx].full_sequence
        ensure_arr(fidx, len(full))
        n = min(len(seq), len(full))
        for i in range(n):
            base_counts[fidx][i][seq[i]] += 1
    if fastq_r2_path and os.path.exists(fastq_r2_path):
        for header, seq, qual in iter_fastq_records(fastq_r2_path):
            ...
            full = frag_map[fidx].full_sequence
            ensure_arr(fidx, len(full))
            aligned = reverse_complement(seq)
            n = min(len(aligned), len(full))
            start = len(full) - n
            for off in range(n):
                base_counts[fidx][start + off][aligned[off]] += 1
    rows = []
    frag_payloads = {}
    for fidx in sorted(frag_map):
        ...
        if expected_len and len(payload) > expected_len:
            payload = payload[:expected_len]
        support = sum(sum(c.values()) > 0 for c in counts)
        overlap = min(len(payload), len(frag.payload))
        mismatch_frac = (sum(1 for a, b in zip(payload[:overlap], frag.payload[:overlap]) if a != b) / max(1, overlap)) if frag.payload else 0.0
        ...
        if support == 0:
            status = "Missing"
        elif frag.checksum and not crc_ok:
            status = "CRC Failed"
        rows.append({
            ...
            "protection_role": getattr(frag, "rs_shard_role", "data"),
            "group_index": getattr(frag, "parity_group_index", 0),
        })
...
def recover_payload_from_fastq(prep: NGSPrepResult, fastq_r1_path: str, fastq_r2_path: Optional[str] = None, method: str = "Consensus reconstruction") -> Tuple[str, Dict[str, Any]]:
    ...
    if protection_mode == "Parity Fragments" and method in ("Consensus + checksum validation", "ECC-assisted recovery", "Consensus + manifest-guided rescue"):
        ...
        for parity in parity_frags:
            covers = [int(x) for x in (parity.parity_covers or []) if int(x) > 0]
            bad = []
            for idx in covers:
                row = row_for(idx)
                if idx not in payloads or (row and row["status"] in ("Missing", "CRC Failed")):
                    bad.append(idx)
            if len(bad) == 1:
                miss = bad[0]
                ...
                if others and parity_payload:
                    ...
                    r = row_for(miss)
                    if r:
                        r["status"] = "Rescued by parity"
                        r["recovered"] = True
                    rescued += 1

    if protection_mode == "Reed-Solomon" and method in ("ECC-assisted recovery", "Consensus + manifest-guided rescue"):
        data_k = int((prep.config or {}).get("rs_data_shards") or 4)
        parity_m = int((prep.config or {}).get("rs_parity_shards") or 2)
        # group fragments by parity_group_index
        groups = {}
        for frag in prep.fragments:
            grp = int(getattr(frag, "parity_group_index", 0) or 0)
            if grp <= 0:
                continue
            groups.setdefault(grp, []).append(frag)
        for grp, frags in groups.items():
            data_frags = sorted([f for f in frags if getattr(f, "rs_shard_role", "data") == "data"], key=lambda x: x.rs_shard_index)
            parity_frags = sorted([f for f in frags if getattr(f, "rs_shard_role", "data") == "parity"], key=lambda x: x.rs_shard_index)
            shards: List[Optional[str]] = []
            missing_data = []
            for f in data_frags:
                row = row_for(f.fragment_index)
                good = f.fragment_index in payloads and (row is None or row["status"] != "CRC Failed")
                if good:
                    shards.append(payloads[f.fragment_index])
                else:
                    shards.append(None)
                    missing_data.append(f.fragment_index)
            for f in parity_frags:
                shards.append(payloads.get(f.fragment_index))
            if 0 < len(missing_data) <= parity_m:
                try:
                    recovered_data = rs_recover_payload_group(shards, data_count=len(data_frags), parity_count=len(parity_frags))
                    for i, f in enumerate(data_frags):
                        row = row_for(f.fragment_index)
                        if f.fragment_index in missing_data:
                            payloads[f.fragment_index] = recovered_data[i][:len(f.payload)]
                            if row:
                                row["status"] = "Rescued by Reed-Solomon"
                                row["recovered"] = True
                            rescued += 1
                except Exception:
                    pass
...
def precheck_decode_payload(dna_payload: str, *, scheme_name: str, mode_codec: str, seed: str, init_dimer: str, whiten: bool, remove_leading_one: bool, target_gc: float, w_gc: float, w_motif: float, ks: Tuple[int, ...]) -> Dict[str, Any]:
    ...
    if not dna:
        return {"ok": False, "error": "Empty DNA payload"}
    try:
...
def decode_dna_payload_to_file(dna_payload: str, *, scheme_name: str, mode_codec: str, seed: str, init_dimer: str, whiten: bool, remove_leading_one: bool, target_gc: float, w_gc: float, w_motif: float, ks: Tuple[int, ...], preferred_stem: str = "recovered_from_fastq") -> Tuple[Dict[str, Any], Optional[str]]:
    ...
    inner, infl = zlib_inflate_until_eof(decoded_buf)
    z_ok = bool(infl.get("eof")) and (infl.get("error") is None)
    out_dir = os.path.join("recovery_out", "ngs_decode", str(uuid.uuid4()))
    ensure_dir(out_dir)
    restored_file = None
    restore_meta = None
    if z_ok:
